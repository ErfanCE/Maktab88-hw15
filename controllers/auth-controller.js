...
const logout = async (req, res, next) => {
	try {
		...
		if (!username?.trim()) {
			return next(new AppError(400, 'use valid username'));
		}
		...
		if (!user) {
			return next(
				new AppError(404, `username: ${username} not found!, R U sure?`)
			);
		}

		if (!user.isLoggedIn) {
			return next(
				new AppError(401, `username: ${username} must logged in first.`)
			);
		}
		...
		console.log(error);
		next(new AppError(500, '[-]: auth controller > logout'));
	}
};
